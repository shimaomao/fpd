...
//					.addString(processName, new String(bpmnBytes)).deploy();
			...
			for (ProcessDefinition processDefinition : list) {
				...
				if(processDefinition.getVersion()>=2){
					//User user = UserUtils.getUser();
					// user.getCompany().getId();
					//新部署之前的最新版本id
					String Procd_id = Db.queryStr("select ID_ from ACT_RE_PROCDEF where KEY_ = ? and VERSION_ = ?", processDefinition.getKey(),processDefinition.getVersion()-1);
			        Db.update("update t_product_biz_cfg set proc_def_id = ? where proc_def_id = ?", processDefinition.getId(),Procd_id);
				}
