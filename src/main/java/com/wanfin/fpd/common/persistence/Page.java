...
public class Page<T> {
	...
	private int pageSize = initPageSize(); // 页面大小，设置为“-1”表示不进行分页（分页无效）
	...
	private Integer initPageSize() {
		Integer initPageSize = null; 
		try {
			String page_pageSize = Global.getConfig("page.pageSize");
			System.out.println(page_pageSize);
			if(StringUtils.isNotEmpty(page_pageSize)){
				initPageSize = Integer.valueOf(Global.getConfig("page.pageSize"));
			}else{
				initPageSize = 30;
			}
		} catch (Exception e) {
	...
	public Page(HttpServletRequest request, HttpServletResponse response,int defaultPageSize,
		...
		//kewenxiu添加
			//一个页面两个分页是区分获取参数
			// 设置页码参数（传递repage参数，来记住页码）
			String pno = request.getParameter("pPageNo");
			if (StringUtils.isNumeric(pno)){
				CookieUtils.setCookie(response, "pPageNo", pno);
				this.setPageNo(Integer.parseInt(pno));
			}else if (request.getParameter("repage")!=null){
				pno = CookieUtils.getCookie(request, "pPageNo");
				if (StringUtils.isNumeric(pno)){
					this.setPageNo(Integer.parseInt(pno));
				}
			}
			// 设置页面大小参数（传递repage参数，来记住页码大小）
			String psize = request.getParameter("pPageSize");
			if (StringUtils.isNumeric(psize)){
				CookieUtils.setCookie(response, "pPageSize", psize);
				this.setPageSize(Integer.parseInt(psize));
			}else if (request.getParameter("repage")!=null){
				pno = CookieUtils.getCookie(request, "pPageSize");
				if (StringUtils.isNumeric(psize)){
					this.setPageSize(Integer.parseInt(psize));
				}
			}else if (defaultPageSize != -2){
			...
			// 设置排序参数
			String porderBy = request.getParameter("porderBy");
			if (StringUtils.isNotBlank(porderBy)){
				this.setOrderBy(porderBy);
			}
	}
	/**
	 * 构造方法
	...
	public Page(int pageNo, int pageSize, long count, List<T> list) {
		/*System.out.println("init1================================"+pageNo);*/
		this.setCount(count);
	...
	public void initialize(){
		/*System.out.println("init11111111========================"+this.pageNo);
		System.out.println("init2222222222========================"+this.pageSize);*/
		//1
		this.first = 1;
...
//	@JsonIgnore
	...
	public Page<T> setList(List<T> list) {	
		System.out.println("this.pageNo111111111=============================="+this.getPageNo());
		System.out.println("this.pageSize111111111=============================="+this.getPageSize());
		initialize();
		if (this.pageNo > this.last) {// 如果当前页大于尾页
			   this.list=null;
		}else{
	...
	public String getJson(){
		...
		p.put("count", getCount());
		p.put("pageSize", getPageSize());
		p.put("pageNo", getPageNo());
		p.put("totalPage", getTotalPage());
		return JSONObject.fromObject(p).toString();
		
	}
	
	public static void main(String[] args) {
		System.out.println(new Page().getJson());
	}
//	/**
