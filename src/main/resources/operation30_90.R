operation30_90=function(loanContractId,applicationMoney,productType,username,password,dbname,host,port){
  productType<-as.numeric(productType)
  ...
  resultTable=dbGetQuery(con,'select user_id,quota,part_loaning_rate1,part_loaning_rate2,update_time,product_type from result_records')
  resultTable$quota<-as.numeric(resultTable$quota)
  resultTable$part_loaning_rate1<-as.numeric(resultTable$part_loaning_rate1)
  resultTable$part_loaning_rate2<-as.numeric(resultTable$part_loaning_rate2)
  resultTable$product_type<-as.numeric(resultTable$product_type)
  ...
  if (n[1] <= 0 || n[2] <= 0){
    ...
    n11=length(resultTable$product_type)
    if (n2 <= 0 || n11 <= 0){
      dbDisconnect(con)
      result<-paste(0,0,0,0,0,0,0,0,0,sep = ",",collapse=",")
      return(result)
      ...
      #calculated quota#
      newest_time<-max(resultTable$update_time[which(resultTable$product_type == productType)])
      n12=length(newest_time)
      if (n12 > 0){
        quota<-resultTable[which(resultTable$user_id %in% user_id & resultTable$update_time == newest_time 
                               & resultTable$product_type == productType),]
        quota[is.na(quota)]<-0
        ...
        if(n3 <= 0 || n4 <= 0 || applicationMoney < 1000 || applicationMoney > 10000 || quota$quota==0 || applicationMoney > quota$quota){
          dbDisconnect(con)
          loanMoneyPart1<-0
          loanMoneyPart2<-0
          successful_or_not<-0
        ...
        }else if (n3 > 0 & n4 > 0 & applicationMoney >= 1000 & applicationMoney <= 10000 & quota$quota > 0 & applicationMoney <= quota$quota){
