RiskControlForOne30_90=function(userID,status,username,password,dbname,host,port){
  ...
  data3=dbGetQuery(con,'select user_id,merchant_id,shop_type from t_merchant')
  ...
  if (n[1] >0 & n[2] > 0){
    data1<-data1[which(data1$user_id %in% userID),]
    ...
    data<-alldata[which(alldata$user_id %in% userID),]
    ...
    if (n2 <= 0){
      ...
      if (0 %in% is_store_currently_trusted) {
        result[1]=0;credibility=0
      } else {result[1]=1;credibility=1}
      ...
      if (data$avg_refund_amount_rate[1] > 0.15 || data$avg_refund_time_rate[1] > 0.15) {
        result[2]=0;refundSituation=0
      } else {result[2]=1;refundSituation=1}
      ...
      if (admittance_operation < 12 || continuous_operation < 6) {
        result[3]=0;operationTime=0
      } else {result[3]=1;operationTime=1}
      ...
      if (n7 > 0) {
        ...
        if (n8 > 0){
          ...
          if (sta_length < summary_length || sta_length == 0 || summary_length == 0){
            result[4]=0;stability=0
          } else {result[4]=1;stability=1}
        }else {result[4]=1;stability=1}
      ...
      if (1 %in% data$is_chargeback){
        result[5]=0;chargeSituation=0
      }else {result[5]=1;chargeSituation=1}
      
      #overdue
      borrowed_data<-data4[which(data4$wish_user_id %in% userID),]
      ...
      if (overdued_or_not > 0){
        result[6]=0;overdueSituation=0
      }else {result[6]=1;overdueSituation=1}
      
      #sales situation
      data$avg_sales[is.na(data$avg_sales)]<-0
      if (data$avg_sales[1] < 1000){
        result[8]=0;paymentSituation=0
      } else {result[8]=1;paymentSituation=1}
      ...
      if (n3 <= 0){
        ...
        shop_type<-data3$shop_type[which(data3$user_id %in% userID)]
        shop_type[is.na(shop_type)]<-0
        shop_type<-as.numeric(shop_type)
        shop_type[is.na(shop_type)]<-0
        if (shop_type[1] == 1 & loaningMoney > 200000){
          loaningMoney = 200000
        } else if (shop_type[1] == 2 & loaningMoney > 1000000){
          loaningMoney = 1000000
        }
      ...
      if (loaningMoney < 1000){
        result[7]=0;withdrawalsSituation=0
      } else {result[7]=1;withdrawalsSituation=1}
      
      
      n=length(result)
      result[is.na(result)]<-0
      n[is.na(n)]=0
      if (sum(result,na.rm=T) == n){
        loaningMoney1<-round(loaningMoney,4)
        loaningMoney2<-round(loaningMoney,4)
        loaningRate1<-round(loaningRate,4)
        feeSum1<-feeSum
        hasBeenPassed<-1
        partLoaningRate1<-round(rate1,4)
        partLoaningRate2<-round(rate2,4)
      } else{
        loaningMoney1<-0
        loaningMoney2<-round(loaningMoney,4)
        loaningRate1<-round(loaningRate,4)
     ...
     if (status == 1){
       isObservationNeeded<-1
     } else {isObservationNeeded<-0}
     resultTable1<-data.frame(userID,hasBeenPassed,loaningMoney1,loaningMoney2,loaningRate1,feeSum1,isObservationNeeded,
     ...
     if (n2 <= 0){
       ...
       data$avg_refund_amount_rate[is.na(data$avg_refund_amount_rate)]<-0
       data$avg_refund_time_rate[is.na(data$avg_refund_time_rate)]<-0
       if (data$avg_refund_amount_rate[1] > 0.10 || data$avg_refund_time_rate[1] > 0.10) {
         result[2]=0;refundSituation=0
       } else {result[2]=1;refundSituation=1}
       ...
       if (n7 > 0) {
         ...
         n8<-length(summary$rate)
         if (n8 > 0){
           sta_length<-length(summary$rate[which(summary$rate >= 0.06)])
           summary_length<-length(summary$rate)
           sta_length[is.na(sta_length)]=0
           summary_length[is.na(summary_length)]=0
           if (sta_length < summary_length || sta_length == 0 || summary_length == 0){
     ...
     if (status == 1){
      ...
      resultTable2<-data.frame(userID,hasBeenPassed,loaningMoney1,loaningMoney2,loaningRate1,feeSum1,isObservationNeeded,
                              credibility,refundSituation,operationTime,stability,chargeSituation,overdueSituation,
                              withdrawalsSituation,paymentSituation,partLoaningRate1,partLoaningRate2)
      ...
      if (n2 <= 0){
        ...
        data$avg_refund_amount_rate[is.na(data$avg_refund_amount_rate)]<-0
        data$avg_refund_time_rate[is.na(data$avg_refund_time_rate)]<-0
        if (data$avg_refund_amount_rate[1] > 0.05 || data$avg_refund_time_rate[1] > 0.05) {
          result[2]=0;refundSituation=0
        } else {result[2]=1;refundSituation=1}
        ...
        if (n7 > 0) {
          ...
          n8<-length(summary$rate)
          if (n8 > 0){
            sta_length<-length(summary$rate[which(summary$rate >= 0.07)])
            summary_length<-length(summary$rate)
            sta_length[is.na(sta_length)]=0
            summary_length[is.na(summary_length)]=0
            if (sta_length < summary_length || sta_length == 0 || summary_length == 0){
      ...
      resultTable3<-data.frame(userID,hasBeenPassed,loaningMoney1,loaningMoney2,loaningRate1,feeSum1,isObservationNeeded,
                              credibility,refundSituation,operationTime,stability,chargeSituation,overdueSituation,
                              withdrawalsSituation,paymentSituation,partLoaningRate1,partLoaningRate2)
