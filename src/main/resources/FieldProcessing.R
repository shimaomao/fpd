FieldProcessing=function(status,username,password,dbname,host,port){
  ...
  if (status == 1){
    ...
    if (n1 > 0){
      ...
      #apply users' summary
      num_in<-aggregate(amount~merchant_id+user_id,data=data1,length)
      required_merchant_id<-num_in[,c("merchant_id","user_id")]
      required_merchant_id<-required_merchant_id[!duplicated(required_merchant_id$merchant_id),]
      ...
      n2=length(required_merchant_id$merchant_id)
      n2[is.na(n2)]<-0
      
      if (n2 > 0){
        
        for (i in 1:n2){
          data<-data1[which(data1$merchant_id %in% required_merchant_id$merchant_id[i]),]
          ...
          if (n3 > 0){
            ...
            if (n4 > 0){
              ...
              if (n == 0){
                ...
                continuousOperation<-length(sales_summary$month)-cid
              } else if (n >= 2){
                k<-n
                repeat {
                  k<-k-1
            ...
            continuous_operation[i]<-as.numeric(continuousOperation)
            ...
            admittance_operation[i]<-as.numeric(admittanceOperation)
            ...
            avg_refund_time_rate[i]<-round(avgRefundTimesRate,4)
            ...
            avg_refund_amount_rate[i]<-round(avgRefundAmountRate,4)
          }
        }
        
        continuous_operation_summary<-data.frame(required_merchant_id,admittance_operation,continuous_operation,avg_sales,avg_refund_time_rate,avg_refund_amount_rate)
        ...
        dbWriteTable(con,"t_merchant_basic_info",continuous_operation_summary,overwrite=T,row.names=F)
        sql1=paste("UPDATE t_merchant o,t_merchant_basic_info r SET o.shop_busiLength=r.admittance_operation WHERE o.merchant_id=r.merchant_id", sep="");
        ips1<-dbGetQuery(con, sql1)
        sql2=paste("UPDATE t_merchant o,t_merchant_basic_info r SET o.last_busiLength=r.continuous_operation WHERE o.merchant_id=r.merchant_id", sep="");
        ips2<-dbGetQuery(con, sql1)
