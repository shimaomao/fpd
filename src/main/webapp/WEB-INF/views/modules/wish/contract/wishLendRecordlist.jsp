	...
	<script type="text/javascript">
        ...
        function toDownload(){
        	...
        	var endtime= $("#endtime").val(); 
       		var url = "${ctx}/wish/contract/wishContract/wishLendRecordlistExcel?customerName="+customerName+"&starttime="+starttime+"&endtime="+endtime;
       	    location.href = url;
        }
        ...
        //发送放款日报表到易联
        function sendLoanDay() {
            $.ajax({
                type: "POST",
                url: "${ctx}/wish/order/wishOrder/loanDay",
                dataType: "json",
                success: function (data) {
                    if (data.flag == 'true') {
                        location.reload(true);
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/wish/contract/wishContract/wishLendRecordlist">放款日对账报表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="tLendingVo" action="${ctx}/wish/contract/wishContract/wishLendRecordlist" method="post" class="breadcrumb form-search">
	...
	<table class="table table-bordered">
		<tr>
			<td>
				<a class="btn btn-primary" onclick="sendLoanDay();">发送放款日报表通知到易联</a>
			</td>
	...
	<table id="contentTable" class="table table-center table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="tLendingVo">
			<tr onclick="$(this).children(':first').children(':first').attr('checked','checked');">
				...
				<td>
					${tLendingVo.customerId}
				</a>
				</td>
				<td>
					${tLendingVo.loanContractId}
				</td>
				<td>
					${tLendingVo.customerName}
				</td>
				<td>
					${tLendingVo.gatheringNumber}
				</td>
				
				<td>
					<fmt:formatNumber value="${tLendingVo.sumLoanAmount}" pattern="#,#00.00#" />
				</td>
				<td>
					<fmt:formatNumber value="${tLendingVo.sumCharge}" pattern="#,#00.00#" />
				</td>
				<td>
					${tLendingVo.lendTime}
				</td>
				<td>
					${tLendingVo.payPrincipalDate}
				</td>
				<%-- <td>
