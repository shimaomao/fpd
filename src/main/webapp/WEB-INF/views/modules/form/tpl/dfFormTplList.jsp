...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/form/tpl/dfFormTpl/">自定义表单列表</a></li>
		<shiro:hasPermission name="form:tpl:dfFormTpl:edit"><li><a href="${ctx}/form/builder/builder/selectModel">表单设计</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="dfFormTpl" action="${ctx}/form/tpl/dfFormTpl/" method="post" class="breadcrumb form-search">
		...
		<c:forEach items="${page.list}" var="dfFormTpl">
			...
			<c:if test="${!fn:contains(dfFormTpl.name, 'BZQL_')}"><tr></c:if>
				...
				<shiro:hasPermission name="form:tpl:dfFormTpl:edit"><td>
    				<c:if test="${dfFormTpl.createBy.id != '1'}">
	    				<a class="btn-edit" href="${ctx}/form/builder/builder/toForm?ftplId=${dfFormTpl.id}&modsub=${dfFormTpl.modsub}" title="设计表单布局">设计</a>
	    				<a href="${ctx}/form/tpl/dfFormTpl/form?id=${dfFormTpl.id}" title="修改基本属性">修改</a>
	    				<a class="btn-view" href="javascript:void(0);" data-id="${dfFormTpl.id}" title="预览表单布局">预览</a>
	    				<%-- <a class="btn-viewData" href="${ctx}/form/tpl/dfFormTpl/viewData?id=${dfFormTpl.id}" title="录入表单布局">录入数据</a> --%>
						<c:if test="${!fn:contains(dfFormTpl.name, '_BZQL')}">
						<%-- <c:if test="${!fn:contains(dfFormTpl.remarks, '禁止删除')}"> --%>
						<a href="${ctx}/form/tpl/dfFormTpl/delete?id=${dfFormTpl.id}" onclick="return confirmx('确认要删除该自定义表单吗？', this.href)">删除</a>
						</c:if>
					</c:if>
					<c:if test="${dfFormTpl.createBy.id == '1'}">
					    <a class="btn-edit" href="${ctx}/form/builder/builder/toForm?ftplId=${dfFormTpl.id}&modsub=${dfFormTpl.modsub}" title="设计表单布局">设计</a>
						<a href="${ctx}/form/tpl/dfFormTpl/isNew?id=${dfFormTpl.id}">保存新模板</a>
						<a class="btn-view" href="javascript:void(0);" data-id="${dfFormTpl.id}" title="预览表单布局">预览</a>
					</c:if>
