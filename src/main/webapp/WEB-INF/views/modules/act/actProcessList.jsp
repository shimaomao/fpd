...
<body>
	...
	<form id="searchForm" action="${ctx}/act/process/" method="post" class="breadcrumb form-search">
		<select id="category" name="category" class="input-medium">
			...
			<c:forEach items="${fns:getOfficeList()}" var="office">
				<c:if test="${office.type == 1 }">
				<option value="${office.id}" ${office.id==category?'selected':''}>${office.name}</option>
				</c:if>
			</c:forEach>
		</select>
	...
	<table class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${page.list}" var="object">
				...
				<tr>
					<td><a href="javascript:updateCategory('${process.id}', '${process.category}')" title="设置分类">${fns:getOfficeById(process.category) == null ? '无分类' : fns:getOfficeById(process.category).name}</a></td>
					<td>${process.id}</td>
					<td>${process.key}</td>
					<td>${process.name}</td>
