	...
	<script type="text/javascript">
		require(['helper/api'], function(api){
			$(function(){
				...
				var $business = $('<div class="control-group"><label class="control-label" for="business">关联业务</label></div>');
				$business.append($('<div class="controls"><div class="input-append"><input id="businessId" name="businessId" value="" type="hidden"><input id="businessName" name="business" readonly="readonly" value="" class="input-large required" onclick="openTreeSelectBox(\'business\',\'businessId\',\'关联业务\',\'\',\'business\',\'\',\'/contract/tLoanContract/treeDataLoanContracts?loanType=2&status=<%=Cons.LoanContractStatus.TO_REVIEW%>\');" type="text"><a id="addressButton" href="javascript:" onclick="openTreeSelectBox(\'business\',\'businessId\',\'关联业务\',\'\',\'business\',\'\',\'/contract/tLoanContract/treeDataLoanContracts?loanType=2&status=<%=Cons.LoanContractStatus.TO_REVIEW%>\');" class="btn">&nbsp;<i class="icon-search"></i>&nbsp;</a></div><span class="help-inline"><font color="red">*</font></span></div>')).append($('<input type="hidden" name="businessTable" value="t_loan_contract">'));
...
<body>
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="mortgage:mortgageContract:edit"><li class="active"><a href="${ctx}/mortgage/mortgageContract/form?id=${data.id}">抵押物品信息${(not empty data.id && fn:contains(data.id, 'new_') == false ) ? '修改' : '添加'}</a></li></shiro:hasPermission>
	</ul>
