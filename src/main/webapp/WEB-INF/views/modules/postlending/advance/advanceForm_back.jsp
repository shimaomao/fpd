	...
	require(['app/repayPlan'], function(rp){
		//初始化提前还款 还款计划--
		   rp.initialize({
				...
				amount : "${nowPrincipal-advance.advanceamount}",//未还本金
				loanRate : "${tLoanContract.loanRate}",
				loanRateType : "${tLoanContract.loanRateType}",
				loanPeriod : "${surplusPeriod}",//还款周期
				//loanDate : '<fmt:formatDate value="${advance.applyDate}" pattern="yyyy-MM-dd"/>',
				loanDate : '<fmt:formatDate value="${advance.applyDate}" pattern="yyyy-MM-dd"/>',
				payType : "${tLoanContract.payType}",//还款方式 	1|2|3 = 等额本息|等额本金|按月付息到期还款
				periodType : "${tLoanContract.periodType}",//（payType=3时才会用到，等额本息和等额本金周期只能是“月”） 1|2|3  = 年|月|日 ,
				...
				payPrincipalDate : "${tLoanContract.payPrincipalDate}",
				payDate : "${advance.applyDate}"
		},"#showplansDiv");
		//alert($("#applyDate").val());
		...
		$(function(){
            ...
            if($.browser.msie){
                  $("#advanceamount").get(0).attachEvent("onpropertychange",function (o){
                      ...
                      if( isNaN( o.target.value ) ){//不是数字
                          ...
                          var  sumAmount = $("#nowPrincipal").val() - o.srcElement.value;
			...
			$("#advanceamount").change(function(){
				...
				if("${tLoanContract.payType}"==4){
					loanPeriod=loanPeriod-1;//按月付息到期还本 重新生成期数需要-1
				}
				...
				rp.initialize({
					...
					amount : leftAmount-advanceamount,//未还本金
					loanRate : "${tLoanContract.loanRate}",
					loanRateType : "${tLoanContract.loanRateType}",
					loanPeriod : loanPeriod,//还款周期
				//	loanDate : '<fmt:formatDate value="${advance.applyDate}" pattern="yyyy-MM-dd"/>',
					loanDate : applyDate,
					payType : "${tLoanContract.payType}",//还款方式 	1|2|3 = 等额本息|等额本金|按月付息到期还款
					periodType : "${tLoanContract.periodType}",//（payType=3时才会用到，等额本息和等额本金周期只能是“月”） 1|2|3  = 年|月|日 ,
					//periodType : loanPeriod,//还款剩余周期（payType=3时才会用到，等额本息和等额本金周期只能是“月”） 1|2|3  = 年|月|日 ,
					payDay : "${tLoanContract.payDay}",//每期还款日期（payType=3时才会用到，等额本息和等额本金的还款日只能是放款日）	为null默认为放款日,
					payOptions : "${tLoanContract.payOptions}",//还款选项（payType=3时才会用到）	1|2|null = 前置付息|一次性付息|常规
		...
		<table class="table-form" style="line-height: 25px;">
		   ...
		   <tr>
			...
			<tr>
				...
				<td>
				  <fmt:formatDate value="${tLoanContract.payDay}" pattern="yyyy-MM-dd"/> 
				</td>
			</tr>	
			  <tr>
				<td class="tit">备注</td>
					...
					<c:if test="${!edit && !empty advance.act.procInsId}">
					  <c:if test="${advance.act.status == 'todo'}">
					    <c:if test="${advance.act.isEnd eq 'Yes'}">
					        <!--  财务处理 -->
					        <input id="cwSubmit" class="btn btn-primary" type="submit" value="同 意"/>&nbsp;
					    </c:if>
					    <c:if test="${advance.act.isEnd ne 'Yes'}">
