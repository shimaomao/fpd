...
// 		}
		...
		function selectMoveOffice(){
			var array = getCheckValue("loanContractId");
			$.ajax({
	         	type: "POST",
	         	url: "${ctx}/contract/tLoanContract/getLoanContracts",
	         	data: {id:array[0]},
	         	dataType: "json",
	         	success: function(data){
	         		if(data[0].status == <%=Cons.LoanContractStatus.TO_LOAN%>){	
	         			 /* if(data[0].moveStatus!=0){
	         				showTip("订单转移审核中，不能重复提交！");
	         			 }else{  */
	         				$('#moveOfficeButton').trigger("click");
	         			//}
	         		}else{
	         			showTip("不是[待放款]状态的合同，不能进行订单转移！");
	         		}
	         	}
	       });
		}
		...
		//添加合同与转移对象的联系
		function moveOfficeTreeselectCallBack(v, h, f){
			var array = getCheckValue("loanContractId");
			var moveOfficeId = $('#moveOfficeId').val();
			if(!moveOfficeId){
				alertx("请选择一个机构!");
				return;
			}
	        var url = "${ctx}/contract/tLoanContract/moveContract?loanContractId="+array[0]+"&officeId="+moveOfficeId+"&tab=bd";
	        location.href = url;
...
<body>
	...
	<div style="display: none;">
		<sys:treeselect id="moveOffice" isAll="false" name="" value="" labelName="moveOfficeName" labelValue="" 
			title="机构对象" url="/sys/office/treeData?type=1&isAll=true" cssClass="" allowClear="true" notAllowSelectParent="true"/>
	</div>
...
<!-- 				<a class="btn btn-primary" onclick="viewLoanContract();">查看合同</a> -->
				<c:if test="${fns:getUser().company.primaryPerson==daikuan}">
	                <a class="btn btn-primary" onclick="loanApply();">放款审批</a>
				...
				<a class="btn btn-primary" onclick="selectMoveOffice();">订单转移</a>
			 </td>
		 </tr>
	  </table>
	</shiro:hasPermission>
