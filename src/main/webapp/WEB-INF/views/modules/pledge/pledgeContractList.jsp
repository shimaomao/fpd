...
<head>
	...
	<script type="text/javascript">
		...
		function toAdd(){
			 var url = "${ctx}/pledge/pledgeContract/form";
				location.href = url;
		}
		function toDelete(){
		    ...
		    var url = "${ctx}/pledge/pledgeContract/delete?id="+array[0];
		...
		function toUpdate(){
			 var array = getCheckValue("cid");
			  ...
			  $.ajax({
		         	type: "POST",
		         	url: "${ctx}/pledge/pledgeContract/getPledgeContractStatus",
		         	data: {id:array[0]},
		         	dataType: "json",
		         	success: function(data){//data要收押物品的数量
		         		if(data.status=='no'){
		         			...
		         			if(data.pleStatus==1 && data.conStatus==1){
				         		  var url = "${ctx}/pledge/pledgeContract/form?id="+array[0];
				       			  location.href = url;
				         		}else if(data.conStatus!=1){
				         			alertx("关联业务合同不是新增状态，不能进行修改");
				         		}else if(data.pleStatus!=1){
				         			alertx("质押物不是新增状态，不能进行修改");
				         		}
		         		   }
		...
		function Getpledge(){
			 ...
			 $.ajax({
		         	type: "POST",
		         	url: "${ctx}/pledge/pledgeContract/getPledgeEntityStatus",
		         	data: {id:array[0]},
		         	dataType: "json",
		         	success: function(data){//data要收押物品的数量
		         		if(data==1){
		         			 $.ajax({
		     		         	type: "POST",
		     		         	url: "${ctx}/pledge/pledgeContract/takeInto",
		...
		function Backpledge(){
			 ...
			 $.ajax({
		         	...
		         	dataType: "json",
		         	success: function(data){//data要收押物品的数量
		         		if(data==<%=Cons.StrutsStatus.TAKE_INTO%>||data==<%=Cons.StrutsStatus.RETURN_YES%>){
		         			 $.ajax({
		     		         	type: "POST",
		     		         	url: "${ctx}/pledge/pledgeContract/changePledgeStatus",
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/pledge/pledgeContract/">质押信息列表</a></li>
		<shiro:hasPermission name="pledge:pledgeContract:edit"><li><a href="${ctx}/pledge/pledgeContract/form">质押信息添加</a></li></shiro:hasPermission>
	</ul>
	...
	<form:form id="searchForm" modelAttribute="pledgeContract" action="${ctx}/pledge/pledgeContract/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>状态：</label>
				<form:input path="struts" htmlEscape="false" maxlength="11" class="input-medium"/>
			</li>
	...
	<table class="table table-bordered">
		<tr><td>
   			...
   			<input id="btnSubmit" class="btn btn-primary" type="submit" onclick="Getpledge();" value="收押"/>
   			...
   			<input id="btnSubmit" class="btn btn-primary" type="submit" onclick="Backpledge();" value="解除"/>
		</td></tr>
    </table>
	<table id="contentTable" class="table table-striped table-bordered table-condensed table-center">
		<thead>
			<tr> 
				...
				<th>更新时间</th>
<!-- 				<th>备注</th> -->
<%-- 				<shiro:hasPermission name="pledge:pledgeContract:edit"><th>操作</th></shiro:hasPermission> --%>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="pledgeContract">
			<tr onclick="$(this).children(':first').children(':first').attr('checked','checked');">
			     <td>
					 <input type="radio" name="cid" id="cId" value="${pledgeContract.id}"/>
				</td>
				<td>
					${pledgeContract.contractNumber}
				</td>
				<td>
					${pledgeContract.customerName}
				</td>
...
<!-- 				<td> -->
<%-- 					${pledgeContract.remarks} --%>
<!-- 				</td> -->
<%-- 				<shiro:hasPermission name="pledge:pledgeContract:edit"><td> --%>
<%--     				<a href="${ctx}/pledge/pledgeContract/form?id=${pledgeContract.id}">修改</a> --%>
<%-- 					<a href="${ctx}/pledge/pledgeContract/delete?id=${pledgeContract.id}" onclick="return confirmx('确认要删除该质押信息吗？', this.href)">删除</a> --%>
