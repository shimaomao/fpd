				...
				$('.nav-tabs').find('li').click(function(){
					...
					$("#pledgeType").change(function(){
						...
						if(sval == 1){//车辆
							...
							loadhref = "${ctx}/collateral/cunhuo/add?dizhiContractId=${data.id}";
						}else if(sval == 3){//股权
							loadhref = "${ctx}/collateral/guquan/add?dizhiContractId=${data.id}";
						}else if(sval == 4){//机器
							loadhref = "${ctx}/collateral/machine/add?dizhiContractId=${data.id}";
						}else if(sval == 5){//无形权力
							loadhref = "${ctx}/collateral/quanli/add?dizhiContractId=${data.id}";
						}else if(sval == 6){//用地
							loadhref = "${ctx}/collateral/yongLand/add?dizhiContractId=${data.id}";
						}
