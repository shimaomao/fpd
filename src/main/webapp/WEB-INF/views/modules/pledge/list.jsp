...
<!DOCTYPE html>
<c:set var='nid' value="${empty param.nid ? 'pledgelist' : param.nid}" />
        ...
        $(function(){        	
        	...
        	var grid = $("#grid_${nid}").jqGrid({
                ...
                colModel: [
                    ...
                    { name: "pledgeType", index: "pledgeType", label: "类型", align:"center", width: 200, hidden: false, search: false, sortable: true,
                    	edittype:'select', formatter:'select', editoptions:{value:"1:车辆;2:存货;3:股权;4:机器设备;5:无形权力;6:用地"}	
                    },
                    ...
                    { name: "edit", index: "edit", label: "修改", align:'center', width: 80, hidden: false, search: false, sortable: true,
                    	formatter: function(cellvalue, options, rowdata){
                    		var url = "${ctx}/pledge/pledgeContract/form.html?"
        			    			+"id="+rowdata.id+"&businessTable=${businessTable}&businessId=${businessId}&nid=${nid}edit";
                    		return "<a href='javascript:void(0);' style='color:#f60' onclick='dialog_show_${nid}(\"" + url + "\",\"编辑质押信息\")'>修改</a>";
                    	}}
                ],
        	...
        	<c:if test="${param.oper!='view'}">
	        	...
	        	grid.navButtonAdd('#gridPager_${nid}',{
			    	...
			    	   onClickButton: function(){ 
			    	      dialog_show_${nid}("${ctx}/pledge/pledgeContract/form.html?"
			    			+"businessTable=${businessTable}&businessId=${businessId}&nid=${nid}select","添加质押信息");
			    	   }, 
			    	   position:"first"
			    });
        	</c:if>
         ...
         function grid_batch_${nid}() {
            ...
            if(confirm("您确定要删除吗?")){
				...
				$.post(
					...
					{ "ids": rows, table : "t_pledge_contract"},
