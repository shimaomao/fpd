...
<c:set var="edit" value="${tEmployee.act.status == 'finish' ? false : true }" />
<html>
<head>
	<title>加入黑名单审批</title>
...
<body>
	...
	<div class="nav-tab-con" style="display:none;">
		<br/>
		<form:form id="inputForm" modelAttribute="tEmployee" action="${ctx}/company/tCompany/saveAudit" method="post" class="form-horizontal">
			<form:hidden path="id"/>
			...
			<c:if test="${tEmployee.act.procDefKey != blackApplyKey}">
				<!-- 解除黑名单流程，显示黑名原因 -->
			...
			<div class="form-actions">
				<c:if test="${edit}">
				<shiro:hasPermission name="contract:tLoanContract:edit"></shiro:hasPermission>
					<c:if test="${empty tEmployee.act.procInsId}">
			...
			<act:histoicFlow procInsId="${tEmployee.act.procInsId}"/>
		</form:form>
	</div>
</body>
</html>