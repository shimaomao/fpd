		...
		<table id="treeTable" class="table table-striped table-bordered table-condensed hide">
			...
			<tbody><c:forEach items="${list}" var="menu">
				<tr id="${menu.id}" pId="${menu.parent.id ne '1'?menu.parent.id:'0'}">
					<td nowrap><i class="icon-${not empty menu.icon?menu.icon:' hide'}"></i><a href="${ctx}/sys/menu/form?id=${menu.id}">
						<c:if test="${fns:getUser().company.primaryPerson==danbao}">
							  <c:choose>
								<c:when  test="${fn:contains(menu.name, '贷前') == true}">
									 保前管理
								</c:when>
								<c:when  test="${fn:contains(menu.name, '贷中管理') == true}">
									 保中管理
								</c:when>
								<c:when  test="${fn:contains(menu.name, '贷后管理') == true}">
									 保后管理
								</c:when>
								<c:otherwise>
								 	${menu.name}
