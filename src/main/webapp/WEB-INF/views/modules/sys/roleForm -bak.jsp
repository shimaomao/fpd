	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function zTreeOnCheck(event, treeId, treeNode) {
		        ...
		        var ids=getChildrens(treeNode);
                $.ajax({  
	                    type : "POST",  //提交方式  
	                    url : "${ctx}/sys/role/updateMenu",//路径  
	                    data : {  
	                    	"id":"${role.id}",
	                        "ids" : ids,
	                        "checked":treeNode.checked//true 勾选，false取消
	                    },
	                    beforeSend:loading('正在提交，请稍等...'),
			...
			function getChildrens(treeNode){
				 ...
				 if(children!=null){
					 for(var i=0;i<children.length;i++){
					     if(children[i]!=null){
					        ids = getChildrens(children[i]) +","+ ids;
					       }  
						 } 
				 }
			     return ids;
			...
			// 用户-菜单
			var zNodes=[
					<c:forEach items="${menuList}" var="menu">{id:"${menu.id}", pId:"${not empty menu.parent.id?menu.parent.id:0}", name:"${not empty menu.parent.id?menu.name:'权限列表'}",checked:"alert('${menu.id}');"},
		            </c:forEach>];
			// 初始化树结构
	...
	<form:form id="inputForm" modelAttribute="role" action="${ctx}/sys/role/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<form:input path="enname" readonly="${role.id != null ? true : false}" htmlEscape="false" maxlength="50" class="required"/>
		...
		<div class="control-group">
			...
			<div class="controls">
				<c:set var="dataScopeDicts" value="${fns:getDictList('sys_data_scope')}"></c:set>
				<form:select path="dataScope" class="input-medium">	
				 <c:if test="${isAdmin}">
					 <c:forEach var="temp" items="${dataScopeDicts}">	
                  ...
                  <c:if test="${isAdmin != true}">
	                   <c:forEach var="temp" items="${dataScopeDicts}">							
	                        <c:if test="${temp.value!='1' && temp.value!='9'}">
								<form:option value="${temp.value}" label="${temp.label}"/>
							</c:if>
	                  </c:forEach>
                  </c:if>
