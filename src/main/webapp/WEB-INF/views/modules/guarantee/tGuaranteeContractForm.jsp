	...
	<script type="text/javascript">
		require(['helper/api'], function(api){
			$(function(){
				...
				$business.append($('<div class="controls"><div class="input-append"><input id="businessId" name="businessId" value="" type="hidden"><input id="businessName" name="business" readonly="readonly" value="" class="input-large required" onclick="openTreeSelectBox(\'business\',\'businessId\',\'关联业务\',\'\',\'business\',\'\',\'/contract/tLoanContract/treeDataLoanContracts?loanType=4&status=<%=Cons.LoanContractStatus.TO_REVIEW%>\');" type="text"><a id="addressButton" href="javascript:" onclick="openTreeSelectBox(\'business\',\'businessId\',\'关联业务\',\'\',\'business\',\'\',\'/contract/tLoanContract/treeDataLoanContracts?loanType=4&status=<%=Cons.LoanContractStatus.TO_REVIEW%>\');" class="btn">&nbsp;<i class="icon-search"></i>&nbsp;</a>&nbsp;&nbsp;</div><span class="help-inline"><font color="red">*</font></span></div>')).append($('<input type="hidden" name="businessTable" value="t_loan_contract">'));
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/guarantee/tGuaranteeContract/form?id=${data.id}">担保信息${not empty data.id?'修改':'添加'}</a></li>
	</ul>
