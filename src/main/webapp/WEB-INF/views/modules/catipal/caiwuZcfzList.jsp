...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/catipal/tCaiwu/listZcfz">小贷资产负债列表</a></li>
		<li><a href="${ctx}/catipal/tCaiwu/form?informFilingType=1">小贷资产负债添加</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="tCaiwu" action="${ctx}/catipal/tCaiwu/listZcfz" method="post" class="breadcrumb form-search">
		...
		<c:forEach items="${page.list}" var="tCaiwu">
			<tr>
				...
				<shiro:hasPermission name="catipal:tCaiwu:edit"></shiro:hasPermission><td>
    				<a href="${ctx}/catipal/tCaiwu/form?informFilingType=1&id=${tCaiwu.id}">修改</a>
					<a href="${ctx}/catipal/tCaiwu/delete?id=${tCaiwu.id}&baoDate=${tCaiwu.baoDate}" onclick="return confirmx('确认要删除该小贷财务报表吗？', this.href)">删除</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
