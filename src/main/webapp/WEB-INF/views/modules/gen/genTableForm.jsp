			...
			<form:form id="inputForm" modelAttribute="genTable" action="${ctx}/gen/genTable/save" method="post" class="form-horizontal">
				...
				<fieldset>
					...
					<div class="control-group">
						<label class="control-label">表名:</label>
						<div class="controls">
							<form:input path="name" htmlEscape="false" maxlength="200" class="required input-medium" readonly="true"/>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label">类名:</label>
						<div class="controls">
							<form:input path="className" htmlEscape="false" maxlength="200" class="required input-medium"/>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label">说明:</label>
						<div class="controls">
							<form:input path="comments" htmlEscape="false" maxlength="200" class="required input-medium"/>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label">By处理:</label>
						<div class="controls">
							<form:select path="isBy" cssClass="input-medium">
					...
					<div class="control-group">
						...
						<div class="controls">
							<form:select path="isApi" cssClass="input-medium">
								<form:options items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
							</form:select>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label">批量处理:</label>
						<div class="controls">
							<form:select path="isBatch" cssClass="input-medium">
								<form:options items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
							</form:select>
						</div>
					</div>
					...
					<div class="control-group">
						<table id="contentTable" class="table table-striped table-bordered table-condensed">
							<thead><tr><th title="数据库字段名">列名</th><th title="默认读取数据库字段备注">说明</th><th title="数据库中设置的字段类型及长度">物理类型</th><th title="实体对象的属性字段类型">Java类型</th>
								...
								<th title="该字段为查询字段时的查询匹配方式">查询匹配</th>
								<c:if test="${genTable.isBy eq '1'}" >
									<th title="选中后该字段生成查询方法">By查询</th>
									<th title="该字段为查询字段生成查询匹配方式">By查询匹配</th>
								</c:if>
								<c:if test="${genTable.isApi eq '1'}" >
									<th title="该字段为API接口属性">API</th>
									<th title="该字段为API接口新增必填属性">API插入</th>
									<th title="该字段为API接口默认展示属性">API查询</th>
								</c:if>
							...
							<c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
								<tr${column.delFlag eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
									...
									<c:if test="${genTable.isBy eq '1'}" >
										<td>
											<input type="checkbox" name="columnList[${vs.index}].isQueryBy" value="1" ${column.isQuery eq '1' ? 'checked' : ''}/>
										</td>
										<td>
											<select name="columnList[${vs.index}].queryByType" class="required input-mini">
												<c:forEach items="${config.queryTypeList}" var="dict">
													<option value="${fns:escapeHtml(dict.value)}" ${fns:escapeHtml(dict.value)==column.queryType?'selected':''} title="${dict.description}">${fns:escapeHtml(dict.label)}</option>
												</c:forEach>
											</select>
									...
									<c:if test="${genTable.isApi eq '1'}" >
										<td>
											<input type="checkbox" name="columnList[${vs.index}].isApi" value="1" ${column.isApi eq '1' ? 'checked' : ''}/>
										</td>
										<td>
											<input type="checkbox" name="columnList[${vs.index}].isApiInsert" value="1" ${column.isApiInsert eq '1' ? 'checked' : ''}/>
										</td>
										<td>
											<input type="checkbox" name="columnList[${vs.index}].isApiQuery" value="1" ${column.isApiQuery eq '1' ? 'checked' : ''}/>
										</td>
									</c:if>
									<td>
