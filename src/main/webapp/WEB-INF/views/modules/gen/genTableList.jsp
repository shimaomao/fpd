...
<body>
	...
	<form:form id="searchForm" modelAttribute="genTable" action="${ctx}/gen/genTable/" method="post" class="breadcrumb form-search">
		...
		<label>父表名：</label><form:input path="parentTable" htmlEscape="false" maxlength="50" class="input-medium"/>
		<label>说明：</label><form:input path="comments" htmlEscape="false" maxlength="50" class="input-medium"/>
		<label>API：</label><form:select path="isApi">
			<form:option value="">全部</form:option>
			<form:options items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
		</form:select>
		<label>By：</label><form:select path="isBy">
			<form:option value="">全部</form:option>
			<form:options items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
		</form:select>
		<label>批量：</label><form:select path="isBatch">
			<form:option value="">全部</form:option>
			<form:options items="${fns:getDictList('yes_no')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
		</form:select>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr>
		<th class="sort-column name">表名</th><th class="sort-column class_name">类名</th><th class="sort-column parent_table">父表</th>
		<th class="sort-column is_by">是否By处理</th><th class="sort-column is_api">是否开启API</th><th class="sort-column is_batch">是否批量处理</th>
		<th>说明</th><shiro:hasPermission name="gen:genTable:edit"><th>操作</th></shiro:hasPermission></tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="genTable">
			<tr>
				...
				<td>${fns:getDictLabel(genTable.isBy, 'yes_no', '')}</td>
				<td>${fns:getDictLabel(genTable.isApi, 'yes_no', '')}</td>
				<td>${fns:getDictLabel(genTable.isBatch, 'yes_no', '')}</td>
