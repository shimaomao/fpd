...
<body>
	...
	<c:set var="stpsItem" value="5" />
	<c:set var="ptagItem" value="productConfigSPPZ"></c:set>
			...
			<tbody>
				<tr>
					...
					<td rowspan="${(fn:length(dqList))+(fn:length(dzList))+(fn:length(dhList))}"  class="atop">
						 <c:forEach items="${windVos}" var="wind" varStatus="idexW">
					    	<span><i class="icon-check"></i>${wind.name}</span><br/><br/>
						</c:forEach>
					</td>
					<td rowspan="${(fn:length(dqList))+(fn:length(dzList))+(fn:length(dhList))}"  class="atop">
						 <c:forEach items="${risks}" var="risk">
						    <c:if test="${risk.status eq 1}">
						    	<span ><input type="checkbox" name="windCheck" value="${risk.id}"/>${risk.name}</span><br/><br/>
						    </c:if>
						</c:forEach>
					</td>
				</tr>
