...
<body>
	...
	<form:form id="inputForm" modelAttribute="menu" action="${ctx}/sys/menu/save" method="post" class="form-horizontal">
		...
		<input type="hidden" name="modeUrl" value="/product/tProduct/productConfigCDPZ?id=${relId }" />
		<%-- <sys:message content="${message}"/> --%>
		<div class="control-group">
			<!-- <label class="control-label">上级菜单:</label> -->
			<div class="controls">
				<input name="parent.id" type="hidden" value="${menu.parent.id}"/>
                <%-- <sys:treeselect id="menu" name="parent.id" value="${menu.parent.id}" labelName="parent.name" labelValue="${menu.parent.name}"
					title="菜单" url="/sys/menu/treeData" extId="${menu.id}" cssClass="required" hideBtn="true" /> --%>
			</div>
		</div>
		<div class="control-group">
